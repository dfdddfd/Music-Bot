...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        .setDescription(`${player.playing ? "▶ 재생 중" : "⏸ 일시정지 됨"} | 채널: **${song.info.author}**`)
        .addField("PROGRESS BAR", `\`${player.duration(player.player.state.position)} ${player.songProgress(message)} ${player.duration(song.info.length)}\``, true)
        .setColor("#2F3136"))
    }
}